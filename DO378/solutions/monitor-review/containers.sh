...
$CMD run -d --name postgresql-conference -p 5432:5432 -e POSTGRESQL_PASSWORD=confi_user -e \
  POSTGRESQL_USER=conference_user -e POSTGRESQL_ADMIN_PASSWORD=conference -e POSTGRESQL_DATABASE=conference \
...
$CMD run -d -p 3000:3000 --net host --name grafana \
  ...
  quay.io/bitnami/grafana:7.3.4 > /dev/null 2>&1
sleep 3
echo
